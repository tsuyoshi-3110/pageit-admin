...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ★ PI を必ず解決（session 文字列 or 展開オブジェクト or retrieve 結果）
    ...
    // ★ G) siteOrders: doc(orderId).set() に保存（互換用キーも同時保存）
    await adminDb
      ...
      .set(
        {
          ...
          payment_intent_id: paymentIntentId, // 互換（snake_case）
          pi: paymentIntentId,                // 互換（簡易キー）
